                                       ...
                                       const THREAD_ID tid) const
{
  ...
  velocity(0) = u->getInternalFaceValue(neighbor, fi, u->getElemValue(elem), correct_skewness);
  if (v)
    velocity(1) = v->getInternalFaceValue(neighbor, fi, v->getElemValue(elem), correct_skewness);
  if (w)
    velocity(2) = w->getInternalFaceValue(neighbor, fi, w->getElemValue(elem), correct_skewness);
