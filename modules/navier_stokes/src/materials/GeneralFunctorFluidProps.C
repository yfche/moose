    ...
    _mu_rampdown(getFunction("mu_rampdown"))
{
  ...
  if (!isParamValid(NS::density))
    addFunctorProperty<ADReal>(NS::density, [this](const auto & r, const auto & t) -> ADReal {
      return _fluid.rho_from_p_T(_pressure(r, t), _T_fluid(r, t));
    });

  addFunctorProperty<ADReal>(NS::cv, [this](const auto & r, const auto & t) -> ADReal {
    return _fluid.cv_from_p_T(_pressure(r, t), _T_fluid(r, t));
  });

  const auto & cp =
      addFunctorProperty<ADReal>(NS::cp, [this](const auto & r, const auto & t) -> ADReal {
        return _fluid.cp_from_p_T(_pressure(r, t), _T_fluid(r, t));
      });

  const auto & mu =
      addFunctorProperty<ADReal>(NS::mu, [this](const auto & r, const auto & t) -> ADReal {
        return _mu_rampdown(r, t) * _fluid.mu_from_p_T(_pressure(r, t), _T_fluid(r, t));
      });

  const auto & k =
      addFunctorProperty<ADReal>(NS::k, [this](const auto & r, const auto & t) -> ADReal {
  ...
  addFunctorProperty<ADReal>(
      NS::time_deriv(NS::density), [this](const auto & r, const auto & t) -> ADReal {
  ...
  addFunctorProperty<ADReal>(NS::Prandtl, [&cp, &mu, &k](const auto & r, const auto & t) -> ADReal {
  ...
  const auto & Re = addFunctorProperty<ADReal>(
      NS::Reynolds, [this, &mu](const auto & r, const auto & t) -> ADReal {
        static constexpr Real small_number = 1e-8;
        return std::max(
            fp::reynolds(
                _rho(r, t), _eps(r, t) * _speed(r, t), _d, std::max(mu(r, t), small_number)),
            1.0);
      });
