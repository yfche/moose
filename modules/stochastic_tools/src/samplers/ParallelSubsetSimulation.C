...
ParallelSubsetSimulation::computeSample(dof_id_type row_index, dof_id_type col_index)
{
  ...
  else
  {
    ...
    if (_sampling_method == "ComponentWiseMH")
    {
      ...
      Real new_sample = AdaptiveMonteCarloUtils::proposeNewSampleComponentWiseMH(
    ...
    else if (_sampling_method == "MH")
    {
      ...
      val = Normal::cdf(Normal::quantile(rnd2, _inputs_sto[col_index].back(), 1), 0, 1);
    }

    else
      mooseError("Invalid sampling method!");
  }
