 ...
 */
template <typename T>
class PiecewiseByBlockLambdaFunctor : public Moose::FunctorImpl<T>
{
public:
...
PiecewiseByBlockLambdaFunctor<T>::PiecewiseByBlockLambdaFunctor(
    ...
    const MooseMesh & mesh,
    const std::set<SubdomainID> & block_ids)
  : Moose::FunctorImpl<T>(clearance_schedule), _name(name), _mesh(mesh)
{
                                           ...
                                           unsigned int state) const
{
  ...
  mooseAssert(it != _elem_from_face_functor.end(), subdomainErrorMessage(elem_from_face.sub_id));
                                           ...
                                           unsigned int state) const
{
  ...
  mooseAssert(it != _face_functor.end(), subdomainErrorMessage(face.sub_id));
...
PiecewiseByBlockLambdaFunctor<T>::evaluate(const Moose::FaceArg & face, unsigned int state) const
{
  ...
  switch (face.limiter_type)
  {
    ...
    default:
      mooseError("Unsported limiter type in PiecewiseByBlockLambdaFunctor::evaluate(FaceArg)");
  }
}
