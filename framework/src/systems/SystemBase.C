                        ...
                        InputParameters & parameters)
{
  ...
  for (THREAD_ID tid = 0; tid < libMesh::n_threads(); tid++)
  {
    ...
    if (auto fe_var = dynamic_cast<MooseVariableFieldBase *>(var.get()))
    {
      ...
      if (auto * const functor = dynamic_cast<Moose::FunctorImpl<ADReal> *>(fe_var))
        _subproblem.addFunctor(name, *functor, tid);
      else if (auto * const functor = dynamic_cast<Moose::FunctorImpl<ADRealVectorValue> *>(fe_var))
        _subproblem.addFunctor(name, *functor, tid);
      else if (auto * const functor = dynamic_cast<Moose::FunctorImpl<RealEigenVector> *>(fe_var))
        _subproblem.addFunctor(name, *functor, tid);
      else
